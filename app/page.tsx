...
export default function Home() {
  ...
  return <main className={sidebarCollapsed ? "app-shell sidebar-is-collapsed" : "app-shell"}>
    <aside className="sidebar">
      ...
      <div className={`cloud-status ${userEmail ? "connected" : ""}`}><i /><div><strong>{userEmail ? "Banco conectado" : cloudReady ? "Banco pronto" : "Banco ainda não conectado"}</strong><small>{userEmail ?? (cloudReady ? "Entre para carregar a base" : "Modo demonstrativo local")}</small></div></div>
    </aside>
    ...
    <section className="workspace prototype-workspace">
      ...
      <div className="architecture-banner"><strong>Fluxo definitivo</strong><span>Extensão → banco protegido → grid. O grid consultará sempre a base completa, não somente a última coleta.</span><em>{cloudReady && userEmail ? "Base online carregada" : cloudReady ? "Banco pronto · entre para acessar" : "Conexão online pendente"}</em></div>
